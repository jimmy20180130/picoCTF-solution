...
public final class DebugProbesImpl {
    ...
    private final void startWeakRefCleanerThread() {
        weakRefCleanerThread = ThreadsKt.thread((21 & 1) != 0, (21 & 2) != 0 ? false : true, (21 & 4) != 0 ? null : null, (21 & 8) != 0 ? null : "Coroutines Debugger Cleaner", (21 & 16) != 0 ? -1 : 0, AnonymousClass1.INSTANCE);
    }
    ...
    private final void updateRunningState(CoroutineStackFrame coroutineStackFrame, String state) {
        ...
        try {
            if (INSTANCE.isInstalled$kotlinx_coroutines_core()) {
                DebugCoroutineInfo cached = callerInfoCache.remove(coroutineStackFrame);
                if (cached != null) {
                    info = cached;
                } else {
                    CoroutineOwner<?> coroutineOwnerOwner = INSTANCE.owner(coroutineStackFrame);
                ...
                info.updateState$kotlinx_coroutines_core(state, (Continuation) coroutineStackFrame);
                CoroutineStackFrame caller = INSTANCE.realCaller(coroutineStackFrame);
                if (caller == null) {
                    return;
                }
    ...
    private final void updateState(CoroutineOwner<?> owner, Continuation<?> continuation, String state) {
        ReentrantReadWriteLock.ReadLock lock = coroutineStateLock.readLock();
        lock.lock();
        try {
            if (INSTANCE.isInstalled$kotlinx_coroutines_core()) {
                owner.info.updateState$kotlinx_coroutines_core(state, continuation);
                Unit unit = Unit.INSTANCE;
            }
        } finally {
            lock.unlock();
    ...
    /* JADX WARN: Multi-variable type inference failed */
    private final <T> Continuation<T> createOwner(Continuation<? super T> completion, StackTraceFrame stackTraceFrame) {
        if (!isInstalled$kotlinx_coroutines_core()) {
            return completion;
        }
        DebugCoroutineInfo info = new DebugCoroutineInfo(completion.getContext(), stackTraceFrame, sequenceNumber$FU.incrementAndGet(debugProbesImpl$SequenceNumberRefVolatile));
        CoroutineOwner owner = new CoroutineOwner(completion, info, stackTraceFrame);
