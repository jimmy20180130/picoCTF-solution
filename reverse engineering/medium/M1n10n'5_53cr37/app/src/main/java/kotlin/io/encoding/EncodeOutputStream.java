...
final class EncodeOutputStream extends OutputStream {
    ...
    @Override // java.io.OutputStream
    public void write(byte[] source, int i, int length) throws IOException {
        Intrinsics.checkNotNullParameter(source, "source");
        checkOpen();
        if (i < 0 || length < 0 || i + length > source.length) {
            throw new IndexOutOfBoundsException("offset: " + i + ", length: " + length + ", source size: " + source.length);
        }
        if (length == 0) {
            return;
        }
