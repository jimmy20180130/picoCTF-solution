...
final class DecodeInputStream extends InputStream {
    ...
    @Override // java.io.InputStream
    public int read(byte[] destination, int i, int length) throws IOException {
        Intrinsics.checkNotNullParameter(destination, "destination");
        if (i < 0 || length < 0 || i + length > destination.length) {
            throw new IndexOutOfBoundsException("offset: " + i + ", length: " + length + ", buffer size: " + destination.length);
        }
        if (this.isClosed) {
        ...
        if (getByteBufferLength() >= length) {
            copyByteBufferInto(destination, i, length);
            return length;
        }
        ...
        while (!this.isEOF && symbolsNeeded > 0) {
            ...
            dstOffset += decodeSymbolBufferInto(destination, dstOffset, length + i, symbolBufferLength);
        }
        if (dstOffset == i && this.isEOF) {
            return -1;
        }
        return dstOffset - i;
    }
